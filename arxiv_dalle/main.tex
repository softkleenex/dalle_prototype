...
\begin{tabular}{@{}c@{\hskip 0.05in} c@{}}
 ...
 \\
\end{tabular}\label{fig:blog_samples1}}%
\hspace{0.75mm}%
\subfloat[an illustration of a baby hedgehog in a christmas sweater walking a dog]{  \setlength\extrarowheight{-3pt}
\begin{tabular}{@{}c@{\hskip 0.05in}c@{}}
 ...
 \\
\end{tabular}\label{fig:blog_samples2}}%
\hspace{0.75mm}%
\subfloat[a neon sign that reads ``backprop''. a neon sign that reads ``backprop''. backprop neon sign]{
  \setlength\extrarowheight{-3pt}
\begin{tabular}{@{}c@{\hskip 0.05in} c@{}}
 ...
 \\
\end{tabular}\label{fig:blog_samples3}}%
\hspace{0.75mm}%
\subfloat[the exact same cat on the top as a sketch on the bottom]{
  \setlength\extrarowheight{-3pt}
\begin{tabular}{@{}c@{\hskip 0.05in}c@{}}
 ...
 \\
\end{tabular}\label{fig:blog_samples4}}%
\caption{With varying degrees of reliability, our model appears to be able to combine distinct concepts in plausible ways, create anthropomorphized versions of animals, render text, and perform some types of image-to-image translation.}
\label{fig:blog_samples}
\end{figure*}
%
% These models use billions of parameters, years of compute accelerator training time, and millions of training data points to 
...
%than as an emergent behavior of a single large generative model.
...
%
\section{Method}
% TODO: mention that we will release dVAE if paper is accepted
...
% This overall procedure can be viewed as maximizing the evidence lower bound (ELB)~\cite{kingma2013auto,rezende2014stochastic} on the likelihood of an image~$x$ given its caption~$y$, by introducing a latent visual codebook $z$ i.e.,
% \begin{equation}
%     \ln p(x \given y) \geqslant \!\!\!\!\!\!\!\! \mathop{\mathbb{E}}_{z \sim q_\phi(z \given x)} \!\!\!\!\! \left[ \ln p_\theta(x \given z) 
%     - \beta\, D_{\mathrm{KL}}(q_\phi(z \given x), p_\psi(z\given y)) \right], \label{eq:elb}
% \end{equation}
% for $\beta=1$ where
% \begin{itemize}
%     \item $p$ denotes the data distribution, 
%     \item $q_\phi$ denotes the $32\times 32$ distribution of visual codes generated by the dVAE encoder given the image $x$
%     \item $p_\theta$ denotes distribtion of the pixels, given the decoder,
%     \item and $p_\psi$ denotes distribution of the visual codes modeled by the autoregressive transformer.
% \end{itemize}
%  $q_\phi$, $p_\theta$  and~$p_\psi$ are all parameterized by neural networks. Note that though the bound only holds for $\beta=1$ in practice we found it helpful to use larger values~\cite{higgins2016beta}.
%
...
% The entropy term is available in closed-form, and the cross-entropy term is a constant.
...
\begin{figure*}[t]
    ...
    \subfloat[FID and IS on MS-COCO as a function of blur radius.]{%
        \begin{tabular}{@{}c}%
        \includegraphics[width=0.31\linewidth]{coco_fid.png} \\
        \includegraphics[width=0.31\linewidth]{coco_is.png}%
        \end{tabular}%
    }\hspace{2mm}
    \subfloat[FID and IS on CUB as a function of blur radius.]{%
        \begin{tabular}{@{}c}%
        \includegraphics[width=0.31\linewidth]{cub_fid.png} \\
        \includegraphics[width=0.31\linewidth]{cub_is.png}%
        \end{tabular}%
    }\hspace{2mm}
    \subfloat[FID and IS on MS-COCO as a function of the sample size used for reranking.]{%
        \begin{tabular}{@{}c}%
        \includegraphics[width=0.31\linewidth]{coco_fid_clip_sample_size.png} \\
...
\begin{figure}[t]
    \centering
    \subfloat[Row attention mask.]{%
        \includegraphics[width=0.24\linewidth]{attn_row.png}%
    }%
    \hspace{2mm}%
    \subfloat[Column attention mask.]{%
        \includegraphics[width=0.24\linewidth]{attn_col.png}%
    }%
    \hspace{2mm}%
    \subfloat[Column attention mask with transposed image states.]{%
        \includegraphics[width=0.24\linewidth]{attn_col_t.png}%
    }%
    \hspace{2mm}%
    \subfloat[Convolutional attention mask.]{%
        \includegraphics[width=0.24\linewidth]{attn_conv.png}%
    }%
...
\end{tabular}}%
\hspace{0.75mm}%
\subfloat[``the exact same photo on the top reflected upside-down on the bottom'']{%
\setlength\extrarowheight{-3pt}
\begin{tabular}{@{}c@{\hskip 0.05in}c@{}}
...
\end{tabular}}%
\hspace{0.75mm}%
\subfloat[``2 panel image of the exact same cat. on the top, a photo of the cat. on the bottom, an extreme close-up view of the cat in the photo.'']{%
\setlength\extrarowheight{-3pt}
\begin{tabular}{@{}c@{\hskip 0.05in}c@{}}
 ...
 \\
\end{tabular}} \\
\subfloat[``the exact same cat on the top colored red on the bottom'']{%
\setlength\extrarowheight{-3pt}
\begin{tabular}{@{}c@{\hskip 0.05in}c@{}}
...
\end{tabular}}%
\hspace{0.75mm}%
\subfloat[``2 panel image of the exact same cat. on the top, a photo of the cat. on the bottom, the cat with sunglasses.'']{%
\setlength\extrarowheight{-3pt}
\begin{tabular}{@{}c@{\hskip 0.05in}c@{}}
...
\end{tabular}}%
\hspace{0.75mm}%
\subfloat[``the exact same cat on the top as a postage stamp on the bottom'']{%
\setlength\extrarowheight{-3pt}
\begin{tabular}{@{}c@{\hskip 0.05in}c@{}}
 ...
 \\
\end{tabular}}%
\caption{Further examples of zero-shot image-to-image translation.}
